...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    ...
    ret: int128 = pt.bias - pt.slope * convert(_timestamp - pt.ts, int128)
    if ret < 0: return 0
    else: return convert(ret, uint256)
    # return convert(ret, uint256) if ret < 0 else 0

    # return convert(max(pt.bias - pt.slope * convert(_timestamp - pt.ts, int128), ZERO128), uint256)
...
def _checkpoint_total_supply():
    ...
    for i in range(20):
        if t > rounded_timestamp:
            ...
            # self.ve_supply[t] = convert(max(pt.bias - pt.slope * dt, ZERO128), uint256)
            tmp: int128 = pt.bias - pt.slope * dt
            if tmp < 0: self.ve_supply[t] = 0
            else: self.ve_supply[t] = convert(tmp, uint256)
        t += WEEK
...
def _claim(addr: address, ve: address, _last_token_time: uint256) -> (uint256, uint256, uint256, uint256, bool):
    ...
    for i in range(50):
        ...
        if week_cursor >= user_point.ts and user_epoch <= max_user_epoch:
            ...
            # balance_of: uint256 = convert(max(old_user_point.bias - dt * old_user_point.slope, ZERO128), uint256)
            balance_of: uint256 = 0
            tmp: int128 = old_user_point.bias - dt * old_user_point.slope
            if tmp > 0: balance_of = convert(tmp, uint256)
            # if old_user_point.bias - dt * old_user_point.slope > 0:
