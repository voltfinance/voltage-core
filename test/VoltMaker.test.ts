...
describe("VoltMaker", function () {
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [["exploiter", this.VoltMakerExploitMock, [this.joeMaker.address]]])
  ...
  describe("setBridge", function () {
    it("does not allow to set bridge for Joe", async function () {
      await expect(this.joeMaker.setBridge(this.joe.address, this.weth.address)).to.be.revertedWith("VoltMaker: Invalid bridge")
    })

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.joeMaker.setBridge(this.weth.address, this.joe.address)).to.be.revertedWith("VoltMaker: Invalid bridge")
    })

    it("does not allow to set bridge to itself", async function () {
      await expect(this.joeMaker.setBridge(this.dai.address, this.dai.address)).to.be.revertedWith("VoltMaker: Invalid bridge")
    })
  ...
  describe("convert", function () {
    ...
    it("reverts if caller is not EOA", async function () {
      await this.joeEth.transfer(this.joeMaker.address, getBigNumber(1))
      await expect(this.exploiter.convert(this.joe.address, this.weth.address)).to.be.revertedWith("VoltMaker: must use EOA")
    })

    it("reverts if pair does not exist", async function () {
      await expect(this.joeMaker.convert(this.mic.address, this.micUSDC.address)).to.be.revertedWith("VoltMaker: Invalid pair")
    })

    it("reverts if no path is available", async function () {
      await this.micUSDC.transfer(this.joeMaker.address, getBigNumber(1))
      await expect(this.joeMaker.convert(this.mic.address, this.usdc.address)).to.be.revertedWith("VoltMaker: Cannot convert")
      expect(await this.joe.balanceOf(this.joeMaker.address)).to.equal(0)
