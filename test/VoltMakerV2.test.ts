...
describe("voltMakerV2", function () {
  ...
  describe("setBridge", function () {
    it("does not allow to set bridge for Joe", async function () {
      await expect(this.joeMakerV2.setBridge(this.joe.address, this.wavax.address)).to.be.revertedWith("VoltMakerV2: Invalid bridge")
    })

    it("does not allow to set bridge for WAVAX", async function () {
      await expect(this.joeMakerV2.setBridge(this.wavax.address, this.joe.address)).to.be.revertedWith("VoltMakerV2: Invalid bridge")
    })

    it("does not allow to set bridge to itself", async function () {
      await expect(this.joeMakerV2.setBridge(this.dai.address, this.dai.address)).to.be.revertedWith("VoltMakerV2: Invalid bridge")
    })
  ...
  describe("convert", function () {
    ...
    it("should convert reflect tokens TRACTOR/AVAX", async function () {
      ...
      this.router.addLiquidityFUSE(this.tractor.address, balance, "0", "0", this.joeMakerV2.address, "11111111111111111", {
        value: "1000000000000000000",
      })
    ...
    it("reverts if caller is not EOA", async function () {
      const exploiterCF = await ethers.getContractFactory("VoltMakerExploitMock")
      ...
      await expect(exploiter.convert(this.joe.address, this.wavax.address)).to.be.revertedWith("VoltMakerV2: must use EOA")
    })

    it("reverts if pair does not exist", async function () {
      await expect(this.joeMakerV2.convert(this.joe.address, this.joeAvax.address)).to.be.revertedWith("VoltMakerV2: Invalid pair")
    })
  })
